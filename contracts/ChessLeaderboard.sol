...
contract ChessLeaderboard {
    ...
    modifier onlyChessContract() {
        require(IChessContract(msg.sender).isChessContract(), "Caller is not a valid Chess contract");
        _;
    }
    ...
    function getStats(address player) external view returns (uint wins, uint losses) {
        PlayerStats storage stats = playerStats[player];
        return (stats.wins, stats.losses);
    }

    function getLeaderboard() external view returns (address[] memory, PlayerStats[] memory) {
